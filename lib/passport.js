...
api.authenticate = function(strategy, options, callback) {
  ...
  if(strategy === 'session') {
    return function(req, res, next) {
      ...
      var done = emitter(callback || function(err) {
        next(err);
      });
    ...
    brIdentity.get(null, user.identity.id, function(err) {
      if(err) {
        if(err.name === 'NotFound') {
          // non-persistent user authenticated
          // if non-persistent users are allowed, persist entire user in session
