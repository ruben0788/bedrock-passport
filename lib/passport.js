...
// TODO: move password strategy to bedrock-idp
...
api.checkAuthentication = function(req, res, callback) {
  ...
  result.session = req.isAuthenicated() ? req.user : false;
 ...
 */
// TODO: move `login` to bedrock-idp
...
function configure(app) {
  ...
  passport.deserializeUser(function(data, callback) {
    ...
    async.auto({
      getIdentity: function(callback) {
        ...
        brIdentity.getIdentity(actor, data.identity, function(err, identity) {
          if(err) {
            return callback(err);
          }
  ...
  // TODO: move passwordstrategy to bedrock-idp
