...
Strategy.prototype.authenticate = function(req) {
  ...
  async.auto({
    ...
    verify: ['validatePublicKey', (results, callback) => {
      const {publicKeyBase58, publicKeyPem} = results.getPublicKey;
      const {signature} = results.parseRequest.params;
      const {signingString: plaintext} = results.parseRequest;
      const verifyOptions = {plaintext, signature};
      if(jsonld.hasValue(
        results.getPublicKey, 'type', 'Ed25519VerificationKey2018')) {
        verifyOptions.algorithm = 'ed25519';
        verifyOptions.publicKeyBase58 = publicKeyBase58;
      } else if(jsonld.hasValue(
        results.getPublicKey, 'type', 'RsaVerificationKey2018')) {
        ...
        return callback(new BedrockError(
          ...
          'HttpSignature.VerifyFailure', {
            'public': true,
            httpStatusCode: 400,
            publicKey: results.getPublicKey
          }));
      }
      httpSignatureCrypto.verify(verifyOptions, (err, verified) => {
        if(err) {
          return callback(new BedrockError(
            'Request signature verify error.',
            'HttpSignature.VerifyError', {}, err));
        }
        if(!verified) {
          return callback(new BedrockError(
...
function getHttpsPublicKey(publicKey, callback) {
  ...
  async.auto({
    ...
    verifyKey: ['dereferencePublicKey', (results, callback) => {
      Promise.resolve(jsigs.checkKey(results.dereferencePublicKey))
        .then(() => callback())
        .catch(err => {
          return callback(new BedrockError(
