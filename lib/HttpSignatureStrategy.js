...
function getPublicKey(publicKey, callback) {
  ...
  async.waterfall([
    function(callback) {
      if(dereferenceUrlScheme.did) {
        return getDidPublicKey(publicKey, function(err, key) {
          if(!err) {
            return callback(err, key);
          }
        });
      ...
      if(dereferenceUrlScheme.http) {
        return getHttpPublickKey(publicKey, function(err, key) {
          if(!err) {
            return callback(err, key);
          }
        });
      ...
      if(dereferenceUrlScheme.https) {
        return getHttpsPublicKey(publicKey, function(err, key) {
          if(!err) {
            return callback(err, key);
          }
        });
      ...
      if(!key) {
        err = new BedrockError('Public key not found.');
      }
      return callback(err, key);
    });
}
...
function getDidPublicKey(publicKey, callback) {
  if(publicKey.substring(0, 4) === 'did:') {
    ...
    didio.get(publicKey, function(err, doc) {
      console.log('getDidPublicKey doc', doc);
      console.log('getDidPublicKey err', err);
      callback(err, doc);
    });*/

    var components = publicKey.split('/');
    if(components[1] === 'keys') {
      var keyNumber = parseInt(components[2]) - 1;
      didio.getDidDocument(components[0], function(err, doc) {
        callback(err, doc.publicKey[keyNumber]);
      });
    }
  }
}
