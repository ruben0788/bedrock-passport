...
// 1. resolve key ID => gives you public key info including key material
// (and owner)
// 2. resolve owner ID and get identity info and list of keys
// 3. make sure key ID is listed in list of keys
...
function getHttpsPublicKey(publicKey, callback) {
  ...
  request.get(publicKey, function(err, res, body) {
    console.log("Recieved publicKey with result", res.body);
    if(err) {
      console.log("ERR", err);
    }
