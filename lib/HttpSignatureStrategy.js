...
Strategy.prototype.authenticate = function(req) {
  ...
  async.auto({
    ...
    getPublicKey: ['checkRequest', function(callback, results) {
      ...
      brKey.getPublicKey(
        publicKey, function(err, publicKeyFound) {
          if(err && err.name === 'NotFound' &&
            bedrock.config.passport.strategies.publicKeyDidLookup) {
            return getDidPublicKey(publicKey.id, function(err, key) {
              return callback(err, key);
            });
          }
          return callback(err, publicKeyFound);
        });
    }],
  ...
  if(publicKey.substring(0, 4) === 'did:') {
    ...
    if(components[1] === 'keys') {
      ...
      didio.getDidDocument(components[0], function(err, doc) {
        console.log('getDidDocument doc', doc);
