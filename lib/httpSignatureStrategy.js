...
async function _validateRequest({req, parsed, options}) {
  // FIXME: need to address internal/external port issues with load balancers
  // so allow `domain` as an acceptable value (do not checking port)
  const expected = [
    bedrock.config.server.host,
    bedrock.config.server.domain
  ];
