 ...
 *
 * @param strategy the name of the strategy to use, `default` to use all
 *          default automatic strategies.
 * @param [required] `true` to throw an error even if authentication was
 *          not attempted in the request (defaults to `false`).
 * @param [options] the options to use if not using `default` as the strategy.
 * @param [optionsMap] if `strategy` is `default`, then this option can
 *          be given as an object of strategy-specific options, keyed by
 *          strategy name.
...
api.createMiddleware = (
  {strategy = 'default', required = false, options, optionsMap}) => {
  let authenticateFn;
  if(strategy === 'default') {
    if(options) {
      throw new Error(
        '"options" cannot be used with "default" strategy; use "optionsMap".');
    }
    options = {
      optional: required !== true,
      strategyOptions: optionsMap || {}
    };
    authenticateFn = ({req, res}) => api.authenticateAll({req, res, options});
  } else {
    if(optionsMap) {
      options = optionsMap[strategy] || {};
    }
    options = Object.assign({}, options || {});
    authenticateFn = ({req, res}) =>
      api.authenticate({strategy, req, res, options});
  }

  return async (req, res, next) => {
    try {
      const {user} = await authenticateFn({req, res});
 ...
 *
 * @deprecated since version 4.0.3, use `createMiddleware` instead.
 *
 * @param [options] the options to use.
 *          [optional] true to only optionally authenticate, defaults to false.
 *          [strategyOptions] an object of strategy-specific options, keyed by
 *            strategy name.
 *            [session] session strategy-options:
 *              [allowUrlEncoded] true to permit session-based authentication
...
api.createAuthenticator = options => {
  ...
  const required = !options.optional;
  console.warn('"createAuthenticator" is deprecated, use "createMiddleware".');
  return api.createMiddleware(
    {strategy: 'default', required, optionsMap: options.strategyOptions || {}});
};
 ...
 */
api.optionallyAuthenticated = api.createMiddleware(
  {strategy: 'default', required: false});
 ...
 * @param next the next route handler.
 */
api.ensureAuthenticated = api.createMiddleware(
  {strategy: 'default', required: true});
